...
const TEMPLATES = [
  ...
  { id: 'doc', label: 'Doc', icon: FileText, method: 'generateDocumentTemplate' },
  { id: 'code', label: 'Code', icon: Code, method: 'generateCodeTemplate' },
  { id: 'research', label: 'Research', icon: Search, method: 'generateResearchTemplate' },
  { id: 'slack', label: 'Message', icon: MessageSquare, method: 'generateSlackMessage' }
]
...
function detectSubtaskAi(text) {
  ...
  const msgWords = ['notify', 'ping', 'reach out', 'contact', 'inform', 'tell', 'slack', 'message', 'dm', 'chat']
...
function PriorityChip({ priority }) {
  ...
  return (
    <span className={`inline-flex items-center px-2 py-px rounded-full text-[10px] font-emphasis ${tone}`}>
      {label}
    </span>
  )
}
...
function renderCommentBody(text) {
  ...
  return parts.map((part, i) =>
    part.startsWith('@') ? (
      <span key={i} className="text-primary font-emphasis">{part}</span>
    ) : (
...
function ActivityList({ taskId, comments }) {
  ...
  useEffect(() => {
    ...
    apiService
      ...
      .then(rows => { if (!cancelled) setChanges(Array.isArray(rows) ? rows : []) })
  ...
  if (loading) return <div className="text-[11px] text-muted-foreground italic px-1 py-2">Loading activity…</div>
  if (error) return <div className="text-[11px] text-destructive px-1 py-2">{error}</div>
  if (merged.length === 0) {
    return <div className="text-[11px] text-muted-foreground italic px-1 py-2">No activity yet.</div>
  }
    ...
    <div className="flex flex-col">
      {merged.map((m, i) => {
        if (m.kind === 'comment') {
          const c = m.payload
          return (
            <div key={`c-${c.id || i}`} className="grid grid-cols-[16px_1fr] gap-2 py-2 text-[12px] leading-snug">
              <span className="mt-1 w-1.5 h-1.5 rounded-full ml-[3px] bg-foreground" aria-hidden="true" />
              <div className="min-w-0">
                <div className="flex items-baseline gap-2">
                  <span className="text-foreground font-emphasis">{c.author_name || 'Someone'}</span>
                  <span className="font-mono text-[10px] text-muted-foreground">{fmtRelative(c.created_at)}</span>
                </div>
                <p className="mt-0.5 text-foreground whitespace-pre-wrap">{renderCommentBody(c.content)}</p>
              </div>
            </div>
          )
        }
        ...
        const valStr =
          c.new_value && c.field_name && String(c.new_value).length < 60
            ? ` → ${c.new_value}` : ''
        return (
          <div key={`a-${c.id || i}`} className="grid grid-cols-[16px_1fr] gap-2 py-1.5 text-[11px] leading-snug">
            <span className={`mt-1 w-1.5 h-1.5 rounded-full ml-[3px] ${isAi ? 'bg-primary' : 'bg-muted-foreground'}`} aria-hidden="true" />
            <div className="min-w-0">
              <span className="text-foreground font-emphasis">{who}</span>{' '}
              <span className="text-muted-foreground">{verb}{fieldStr}{valStr}</span>
              <span className="text-muted-foreground/70 font-mono ml-2 text-[10px]">{fmtRelative(c.created_at)}</span>
            </div>
          </div>
        )
      })}
...
function SourceTab({ task }) {
  ...
  if (!task?.meetingId) {
    return <div className="text-[11px] text-muted-foreground italic px-1 py-2">No source meeting. This task was created manually.</div>
  }
  return (
    <div className="space-y-2 text-[12px]">
      ...
      <div className="rounded-md border border-primary/40 bg-primary/5 p-3">
        <div className="font-serif text-[14px] text-foreground mb-1">{meeting?.name || 'Meeting'}</div>
        <div className="font-mono text-[10px] text-muted-foreground">{meeting?.id || task.meetingId}</div>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-4">
        ...
        <div>
          ...
          <div className="space-y-1">
            {aiSuggestions.map(({ task: t, kind }) => (
              <div key={t.id} className="flex items-center gap-2 px-2 py-1.5 rounded-sm border border-primary/30 bg-primary/5">
                <button
                  type="button"
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => { acceptAiSuggestion(task.id, t.id, kind); addNotification({ type: 'success', message: 'Linked' }) }}
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => { rejectAiSuggestion(task.id, t.id, kind); addNotification({ type: 'info', message: 'Dismissed' }) }}
      ...
      <div>
        ...
        {linkedRows.length === 0 ? (
          <div className="text-[11px] text-muted-foreground italic">No linked tasks.</div>
        ) : (
          <div className="space-y-1">
            {linkedRows.map(t => (
              <div key={t.id} className="group flex items-center gap-2 px-2 py-1.5 rounded-sm border border-border hover:bg-muted">
      ...
      <div>
        ...
        {query.trim() && (
          <div className="mt-1 border border-border rounded-sm bg-popover overflow-hidden">
            {available.length === 0 ? (
              <div className="px-2 py-1.5 text-[11px] text-muted-foreground italic">No matches.</div>
            ) : (
              available.map(t => (
                <button
                  key={t.id}
                ...
                >
                  <StatusDot status={t.status} size={10} />
                  <span className="flex-1 truncate text-foreground">{t.title || 'Untitled'}</span>
                </button>
              ))
            )}
          </div>
...
export default function TaskInspector() {
  ...
  useEffect(() => {
    ...
    apiService.getTaskComments(task.id)
      .then(rows => { if (!cancelled) setServerComments(Array.isArray(rows) ? rows : []) })
      .catch(() => { if (!cancelled) setServerComments([]) })
    apiService.getUsers()
  ...
  }, [currentTaskId, tasks, closeTaskInspector, openTaskInspector, showMentionDropdown])
  ...
  const setStatus = status => task && status !== task.status && updateTask(task.id, { status })
  const setPriority = priority => task && priority !== task.priority && updateTask(task.id, { priority })
  const setDueDate = dueDate => task && dueDate !== (task.dueDate || '') && updateTask(task.id, { dueDate: dueDate || null })
  ...
  const addSubtaskRow = () => {
    ...
    const next = [...(task.subtasks || [])]
    next.push({ id: `subtask-${Date.now()}`, text: newSubtask.trim(), completed: false })
    updateTask(task.id, { subtasks: next })
    setNewSubtask('')
  }
  const promoteSubtask = (subtask) => {
    ...
    if (created?.id) {
      addNotification({ type: 'success', message: `Created task "${subtask.text}"` })
    }
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addNotification({ type: 'error', message: `Failed to delete task: ${e.message || 'server error'}` })
    }
  }
  ...
  const runTemplate = async (templateId, contextText) => {
    ...
    try {
      ...
      const result = await openaiService[def.method](ctx, contextText || task.title || '')
      setAiContent({ title: `AI ${def.label}`, content: result, template: templateId })
    } catch (e) {
      addNotification({ type: 'error', message: `${def.label} generation failed: ${e.message}` })
    } finally {
  ...
  const generateSubtasks = async () => {
    ...
    try {
      ...
      if (updated.subtasks && Array.isArray(updated.subtasks)) {
        ...
        addNotification({ type: 'success', message: `Generated ${merged.length} subtasks` })
      } else {
        addNotification({ type: 'info', message: 'AI returned no new subtasks' })
      }
    } catch (e) {
      addNotification({ type: 'error', message: `Subtask generation failed: ${e.message}` })
    } finally {
      setLoadingAiAction(null)
    }
  }
  ...
  const applyContext = async () => {
    if (!task || !contextDraft.trim()) return
    ...
    try {
      ...
      const updated = await openaiService.updateTaskWithContext(ctx, contextDraft.trim())
      const patch = {}
      if (updated.title && updated.title !== task.title) patch.title = updated.title
      if (updated.description && updated.description !== task.description) patch.description = updated.description
      if (updated.priority && updated.priority !== task.priority) patch.priority = updated.priority
      if (updated.status && updated.status !== task.status) patch.status = updated.status
      if (updated.dueDate && updated.dueDate !== task.dueDate) patch.dueDate = updated.dueDate
      if (Array.isArray(updated.assignees)) patch.assignees = updated.assignees
      ...
      if (Object.keys(patch).length > 0) {
        ...
        addNotification({ type: 'success', message: `Task updated (${Object.keys(patch).length} fields changed)` })
      } else {
      ...
      addNotification({ type: 'error', message: `Context update failed: ${e.message}` })
    } finally {
      setLoadingAiAction(null)
    }
  }
  ...
  const handleCommentKeyDown = (e) => {
    if (showMentionDropdown && filteredMentionUsers.length > 0) {
      ...
      if (e.key === 'ArrowUp') {
        e.preventDefault()
        setSelectedMentionIdx(i => (i - 1 + filteredMentionUsers.length) % filteredMentionUsers.length)
        return
      }
  ...
  const submitComment = async () => {
    ...
    try {
      ...
      addNotification({ type: 'error', message: `Failed to post comment: ${e.message}` })
    }
  }

  const copyAiContent = async () => {
    if (!aiContent?.content) return
    try {
      await navigator.clipboard.writeText(aiContent.content)
      addNotification({ type: 'success', message: 'Copied to clipboard' })
    } catch {
      addNotification({ type: 'info', message: 'Clipboard unavailable — select the text manually' })
    }
  }
  const applyAiContentToDescription = () => {
    if (!aiContent?.content || !task) return
    const merged = (descriptionDraft ? descriptionDraft + '\n\n' : '') + aiContent.content
    ...
    return (
      <div className="flex flex-col h-full p-6 text-center">
        <div className="m-auto max-w-[240px]">
          ...
          <Button variant="outline" size="sm" className="mt-4" onClick={closeTaskInspector}>Close</Button>
        </div>
      </div>
    )
  }
  ...
  const hasLinks =
    ...
    (task.aiDiscoveredLinks?.length || 0) > 0

  const tabs = [
    { id: 'detail', label: 'Detail' },
    { id: 'activity', label: 'Activity' },
    { id: 'linked', label: hasLinks ? `Linked · ${(task.linkedTasks?.length || 0) + (task.aiCreatedLinks?.length || 0) + (task.aiDiscoveredLinks?.length || 0)}` : 'Linked' },
  ...
  return (
    <div className="flex flex-col h-full bg-card text-card-foreground">
      {/* Header */}
      <header className="flex items-center gap-2 px-3 border-b border-border flex-shrink-0" style={{ height: 'var(--topbar-h)' }}>
        <button
          type="button"
        ...
        <span className="font-mono text-[11px] text-muted-foreground truncate">{task.id?.slice(0, 12) || 'new'}</span>
        ...
        {tabs.map(t => (
          <button
            ...
            className={
              ...
              (activeTab === t.id ? 'text-foreground border-primary' : 'text-muted-foreground border-transparent hover:text-foreground')
            }
          >
            {t.label}
          </button>
      ...
      <div className="flex-1 overflow-auto px-4 py-4 space-y-4 min-w-0">
        {activeTab === 'detail' && (
          <>
            ...
            <div className="space-y-2">
              ...
              <Field label="Project">
                <span className="text-[12px] text-muted-foreground">{projectName}</span>
              </Field>
              {isAi && (
                <Field label="Source">
                  <AiBadge />
            ...
            {task.aiMetadata?.summary && (
              <div className="rounded-md border border-primary/40 bg-primary/5 p-3">
                <div className="flex items-center gap-1.5 mb-1.5">
                  <AiBadge />
                  <span className="text-[10px] uppercase tracking-[0.04em] text-primary font-emphasis">AI summary</span>
                </div>
                <p className="font-serif text-[13px] text-foreground leading-snug">{task.aiMetadata.summary}</p>
                {task.aiMetadata.source && (
                  <p className="mt-1.5 font-mono text-[10px] text-muted-foreground">{task.aiMetadata.source}</p>
                )}
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center gap-1.5">
              ...
              >
                ...
                {loadingAiAction === 'subtasks' ? 'Generating…' : 'Generate subtasks'}
              </Button>
              <Button
                variant="outline"
                size="sm"
              ...
              <div className="rounded-md border border-primary/40 bg-primary/5 p-2 space-y-2">
                ...
                <div className="flex items-center justify-end gap-1.5">
                  ...
                  <Button
                    ...
                    disabled={!contextDraft.trim() || loadingAiAction === 'context'}
            ...
            {/* Subtasks */}
            <div>
              <div className="text-[10px] uppercase tracking-[0.06em] text-muted-foreground font-emphasis mb-1.5">Subtasks</div>
              <div className="flex flex-col">
              ...
              <div className="flex items-center gap-1.5 mt-1.5">
                <input
                  ...
                  onKeyDown={e => { if (e.key === 'Enter' && newSubtask.trim()) { e.preventDefault(); addSubtaskRow() } }}
                ...
                />
                <Button variant="ghost" size="sm" onClick={addSubtaskRow} disabled={!newSubtask.trim()} className="h-7 px-2">
            ...
            {/* Description */}
            <div>
              <div className="text-[10px] uppercase tracking-[0.06em] text-muted-foreground font-emphasis mb-1.5">Description</div>
              <textarea
            ...
            {aiContent && (
              <div className="rounded-md border border-primary/40 bg-primary/5 p-3 space-y-2">
                <div className="flex items-center gap-1.5">
                  <AiBadge />
                  <span className="text-[11px] font-emphasis text-foreground">{aiContent.title}</span>
                  <span className="flex-1" />
                  <button
                    type="button"
                ...
                <div className="flex items-center justify-end gap-1.5">
                  <Button variant="ghost" size="sm" onClick={copyAiContent} className="h-7 px-2 text-[11px] gap-1">
                    <Copy className="h-3 w-3" />
                    Copy
                  </Button>
                  <Button size="sm" onClick={applyAiContentToDescription} className="h-7 px-2 text-[11px]">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-[0.06em] text-muted-foreground font-emphasis mb-1.5">
                Comments {serverComments.length > 0 && <span className="font-mono text-muted-foreground/70">· {serverComments.length}</span>}
              </div>
              <div className="space-y-2 mb-2">
                {serverComments.map(c => (
                  <div key={c.id} className="rounded-sm border border-border bg-background p-2">
                    <div className="flex items-baseline gap-2 mb-0.5">
                      <span className="text-[11px] font-emphasis text-foreground">{c.author_name || 'Someone'}</span>
                      <span className="font-mono text-[10px] text-muted-foreground">{fmtRelative(c.created_at)}</span>
                    </div>
                    ...
                    {filteredMentionUsers.map((u, i) => (
                      <button
                        ...
                        className={
                          ...
                          (i === selectedMentionIdx ? 'bg-secondary text-foreground' : 'text-foreground hover:bg-muted')
                        }
                      >
        ...
        {activeTab === 'activity' && <ActivityList taskId={task.id} comments={serverComments} />}
      ...
      <footer className="flex items-center gap-2 px-3 py-2 border-t border-border flex-shrink-0">
        <Button
          ...
          onClick={() => { setActiveTab('detail'); setTimeout(() => commentRef.current?.focus(), 0) }}
        >
          Comment
        </Button>
