...
export default function TasksView({ tasks: tasksProp, emptyMessage } = {}) {
  ...
  const groups = useMemo(() => {
    ...
    for (const arr of buckets.values()) arr.sort(sortRows)
  ...
  const visibleIds = useMemo(() => {
    ...
    for (const g of groups) {
      if (collapsed[g.value]) continue
      for (const t of g.items) ids.push(t.id)
    }
    return ids
  }, [groups, collapsed])
  ...
  useEffect(() => {
    if (currentTaskId) return // Inspector handles j/k
    const onKey = e => {
